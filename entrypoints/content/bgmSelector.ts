...
const BgmSelector = (responsetext: any) => {
  ...
  const observedTexts = responsetext.musics.map(({music, start_text}) => {
    ...
    const randomBgm =
      availableBgm[Math.floor(Math.random() * availableBgm.length)];
    
      usedBgm.add(randomBgm);
    return { title: randomBgm.title, artist: randomBgm.artist , src: randomBgm.src, tag: randomBgm.tag, start_text: start_text };
